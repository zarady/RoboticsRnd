...
def inverse_kinematics_numerical(target, initial_guess, lr=0.1, max_iter=1000, tol=1e-2):
    """
    Very simple numerical IK (position only, no orientation).
    target = (x, y, z)
    initial_guess = list of 6 joint angles (degrees)
    lr = learning rate
    max_iter = maximum iterations
    tol = acceptable position error
    """
    ...
    for _ in range(max_iter):
        ...
        pts = forward_kinematics_from_thetas(thetas)
        end_eff = np.array(pts[-1])
        error = target - end_eff

        if np.linalg.norm(error) < tol:
            break  # converged

        # Compute numerical Jacobian (finite difference)
        J = np.zeros((3, 6))
        delta = 1e-3
        for i in range(6):
            thetas_perturbed = thetas.copy()
            thetas_perturbed[i] += delta
            eff_perturbed = np.array(forward_kinematics_from_thetas(thetas_perturbed)[-1])
            J[:, i] = (eff_perturbed - end_eff) / delta

        # Update using Jacobian transpose
        dtheta = lr * J.T @ error
        thetas += np.degrees(dtheta) * 0.01  # scale update
...
class RobotApp(QWidget):
    def __init__(self):
        ...
        # End Effector Position (editable + buttons)
        ee_group = QGroupBox("End Effector Position (x, y, z)")
        ee_layout = QGridLayout()
        self.ee_boxes = []
        self.ee_step = 1.0  # step size for up/down buttons

        for i, axis in enumerate(["x", "y", "z"]):
            label = QLabel(f"{axis}:")
            ee_layout.addWidget(label, i, 0)

            box = QLineEdit("0.0")
            box.setFixedWidth(80)
            box.editingFinished.connect(self.update_from_ee)
            ee_layout.addWidget(box, i, 1)
            self.ee_boxes.append(box)

            # Up button
            up_btn = QPushButton("▲")
            up_btn.setFixedWidth(25)
            up_btn.clicked.connect(lambda _, idx=i: self.nudge_ee(idx, +self.ee_step))
            ee_layout.addWidget(up_btn, i, 2)

            # Down button
            down_btn = QPushButton("▼")
            down_btn.setFixedWidth(25)
            down_btn.clicked.connect(lambda _, idx=i: self.nudge_ee(idx, -self.ee_step))
            ee_layout.addWidget(down_btn, i, 3)
    ...
    def update_from_ee(self):
        try:
            x = float(self.ee_boxes[0].text())
            y = float(self.ee_boxes[1].text())
            z = float(self.ee_boxes[2].text())
            target = (x, y, z)
        except ValueError:
            QMessageBox.warning(self, "Invalid Input", "Please enter numeric XYZ values.")
            return

        # Perform inverse kinematics
        new_angles = inverse_kinematics_numerical(target, self.angles)

        # Clamp angles to valid ranges
        for i in range(6):
            rng = DH[i+1][3]
            min_r, max_r = rng
            new_angles[i] = max(min_r, min(max_r, new_angles[i]))
        ...
        for i in range(6):
            ...
            self.angle_boxes[i].setText(f"{self.angles[i]:.1f}")

        # Replot
        self.update_plot()
        
    def nudge_ee(self, idx, delta):
        """Increase or decrease one of x/y/z by delta, then update robot."""
        try:
            current_val = float(self.ee_boxes[idx].text())
        except ValueError:
            current_val = 0.0

        new_val = current_val + delta
        self.ee_boxes[idx].setText(f"{new_val:.2f}")
        self.update_from_ee()
